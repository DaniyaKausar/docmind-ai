    ...
    if uploaded_file:
        if uploaded_file.name != st.session_state.pdf_name:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                chain_future = executor.submit(build_qa_chain, vectorstore)
                summary_future = executor.submit(generate_document_summary, vectorstore)
                st.session_state.chain_and_retriever = chain_future.result()
